...
public class EOSNetcodeTransport : NetworkTransport
{
    ...
    [SerializeField]
    EOSServer _serverPrefab;
    [SerializeField]
    EOSClient _clientPrefab;
    ...
    public void StartServer(string localPUID,string socketName)
    {
    ...
    public void StartClient(string localPUID, string remotePUID, string socketName)
    {
        var _eosNet = SingletonMonoBehaviour<EOSNet>._instance;
        _eosNet.GetComponent<EOSNetcodeTransport>().InitializeEOSClient(localPUID, remotePUID, socketName, 0);
        NetworkManager.Singleton.StartClient();
    }
    public void StartHost(string localPUID, string socketName)
    {
        var _eosNet = SingletonMonoBehaviour<EOSNet>._instance;
        _eosNet.GetComponent<EOSNetcodeTransport>().InitializeEOSServer(localPUID, socketName, 0);
    ...
    public override void Send(ulong transportID, ArraySegment<byte> payload, NetworkDelivery networkDelivery)
    {
        ...
        if (payload.Count > P2PInterface.MaxPacketSize)
        {
            ...
            {
                Debug.Log($"NGO : send fail payload {payload.Count} exceeds {P2PInterface.MaxPacketSize}");
                return;
            }
        }
    ...
    public int GetNewConnectID(EOSNet.Role role)
    {
        m_NextTransportID++;
        return m_NextTransportID;
    }
    void InitializeEOSServer(string localPUID, string socketid, byte channel)
    {
    ...
    void OnServerReceivedPacketCB((int connectid, EOSNet.EOS_Packet packet) info)
    {

    }
    void OnServerStateChangedCB(EOSPeer.state state)
    {

    }
    void InitializeEOSClient(string localPUID, string remotePUID, string socketid, byte channel)
    {
    ...
    void OnClientReceivedPacketCB(EOSNet.EOS_Packet packet)
    {

    }
    void OnClientStateChangedCB(EOSPeer.state state)
    {
