...
public class EOSServer : EOSPeer
{
    ...
    public event Action<(int, EOSNet.EOS_Packet)> _onReceivedPacket;
    public event Action<(int, EOS_Socket.Connection)> _onConnectionStateChanged;

    public delegate int GetNewConnectID(EOSNet.Role role);
    ...
    public bool GetConnection(int id, out EOS_Socket.Connection connection)
    {
        ...
        if (GetPUID(id, out var puid))
        {
            if (_localPUID == puid)
            {
                return _socket.GetConnection(EOSNet.Role.localClient, puid, out connection);
            }
            else
            {
                return _socket.GetConnection(EOSNet.Role.RemotePeer, puid, out connection);
            }
        }
        return false;
    }
    ...
    public void SendToClient(byte channelId, ArraySegment<byte> segment, int connectionId, PacketReliability reliability)
    {
        if (GetPUID(connectionId, out var puid))
        {
            if(puid == _localPUID)
            {
                _eosNet.SendLocal(_socket, EOSNet.Role.localClient, _channel, segment);
    ...
    {
        var remoterole = (EOSNet.Role)info._remoteRole;
        if (remoterole != EOSNet.Role.localHost)
        {
    ...
    {
        var remoterole = (EOSNet.Role)info.ClientData;
        if (remoterole != EOSNet.Role.localHost)
        {
    ...
    public bool StopConnection(string puid)
    {
        if (_localPUID == puid)
        {
            _socket.StopConnect(EOSNet.Role.localClient, puid);
        }
        else
        {
            _socket.StopConnect(EOSNet.Role.RemotePeer, puid);
        }
        RemoveMapping(puid);
        return true;
    }
